...
class WardController extends Controller
{
    ...
    public function index(){
        ...
        $docs=User::where(function ($query) {
            $query->where('user_type', 'LIKE', '%'.'doctor' . '%')
                  ->orWhere('user_type', '=', 'admin');
        })->get();

        $nurses = User::where(function ($query) {
            $query->where('user_type', 'LIKE', '%'.'nurse' . '%')
                  ->orWhere('user_type', '=', 'admin');
        })->get();
        $data=DB::table('wards')->get();
        return view('ward.index',compact("title","docs","data","nurses"));
    }

    public function createWard(Request $request){
        $ward=new Ward();
        $ward->doctor_id=$request->doctor;
        $ward->ward_title=$request->ward_title;
        $ward->wing=$request->ward_wing;
        $ward->ward_no=$request->ward_num;
        $ward->beds=$request->total_beds;
        $ward->free_beds=$request->free_beds;
        $ward->charges_per_day=$request->ward_charge;
        $ward->nurse_incharge=$request->nurse_incharge;
