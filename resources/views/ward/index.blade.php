        ...
        <div class="box box-info">
            ...
            <form method="POST" action="{{route('add-ward')}}" class="form-horizontal">
                ...
                <div class="box-body">
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="ward_num">{{__('Ward Title')}}<span style="color:red">*</span></label></label>

                        <div class="col-sm-10">
                            <input class="form-control" name="ward_title" required id="ward_title" type="text"
                                placeholder="Ward Title">
                        </div>
                    </div>

                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="ward_wing">{{__('Ward Wing')}}<span style="color:red">*</span></label></label>

                        <div class="col-sm-10">
                            <input class="form-control" name="ward_wing" required id="ward_title" type="text"
                                placeholder="Ward Wing">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="ward_num">{{__('Charges Per Day')}}<span style="color:red">*</span></label></label>

                        <div class="col-sm-10">
                            <input class="form-control" name="ward_charge" required id="ward_charge" type="number"
                                placeholder="Charges Per Day (KSH)">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="nurse_incharge">{{__('Nurse in-charge')}}<span
                                style="color:red">*</span></label></label>

                        <div class="col-sm-10">
                            <select class="form-control" name="nurse_incharge" id="nurse_incharge">
                                @foreach ($nurses as $nurse)
                                <option value="{{$nurse->id}}">{{ucWords($nurse->name)}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div id="example1_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
                    ...
                    <table id="example1" class="table table-bordered table-striped dataTable" role="grid"
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{$app->ward_no}}</td>
                                <td>{{ $app->ward_title }}</td>
                                <td>{{ $app->wing }}</td>
                                <td>{{$app->beds}}</td>
                                <td>{{$app->free_beds}}</td>
                                <td>Dr. {{ucWords(App\User::find($app->doctor_id)->name)}}</td>
                                <td>{{ucWords(App\User::find($app->nurse_incharge)->name)}}</td>
                            </tr>
                            @endforeach
                        </tbody>
