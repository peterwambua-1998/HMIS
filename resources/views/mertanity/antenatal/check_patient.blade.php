        ...
        <div style="height:46.5vh;overflow-y:scroll;overflow-x:hidden;" class="mt-2 mb-1 box box-dark">
            ...
            <div class="box-body">
                ...
                @if ($antenatal !== null)
                    ...
                    @foreach ($antenatal as $item)
                    
                    <h4>Doc Note: <span class="text-green">{{ $item->doc_notes }}</span><small> ({{$item->created_at}})</small></h4>
                    <h4>Baby Health: <span class="text-green">{{ $item->baby_health }}</span><small> ({{$item->created_at}})</small></h4>
        ...
        <div class="box mt-2 box-dark">
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    ...
                    <div class="box-header with-border">
                        <p class="h4 m-0">Baby Health</p>
                    </div>
                    <div class="box-body">
                        <div class="form-group">
                            <textarea class="form-control text-capitalize" name="baby_health" id="baby_health" cols="73"
                                rows="10"></textarea>
                        </div>
                    </div>
                </div>
...
<div class="box box-info">
    ...
    <!-- /.box-header -->
    <!-- form start -->
    <form  action="{{ route('inpatientmertanityregister') }}" method="post" class="form-horizontal">
        {{csrf_field()}}
        ...
        <div class="box-body">
            <div class="form-group">
                <label for="dis1" class="col-sm-2 control-label">{{__('Gravidae')}}</label>
                <div class="col-sm-10">
                    <input type="text" required class="form-control" id="dis1" placeholder="Enter Gravidae"
                    name="gravidae" />
                </div>
            </div>

            <div class="form-group">
                <label class="col-sm-2 control-label">{{__('No. of ANC Visits')}}<span style="color:red">*</span></label>
                <div class="col-sm-2">
                    <div class="input-group">
                        ...
                        <input type="number" required min="1" step="1" data-number-to-fixed="2" data-number-stepfactor="100"
                            class="form-control currency" name="num_anc" id="dis2">
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                <label for="dis3" class="col-sm-2 control-label">{{__('Date of Last Menstrual Period (LMP):')}}<span style="color:red">*</span></label>
                <div class="col-sm-4">
            ...
            <div class="form-group">
                <label for="dis3" class="col-sm-2 control-label">{{__('Expected Date of Delivery (EDD):')}}<span style="color:red">*</span></label>
                <div class="col-sm-4">
                    <input type="date" class="form-control" name="expected_day">
                </div>
            </div>
                ...
                callback:function(result){
                    if (result) {
                        ...
                        if ("{{ $user->user_type}}" == 'admin') {
                            var amount = 1500;
                            var billing_for = "antenatal";
                        } else {
                            var amount = 1500;
                            var billing_for = "antenatal";
                        }
                        
                        var baby_health = $('#baby_health').val();
                        ...
                        var data={
                            ...
                            completed: completed,
                            department: dept,
                            babyhealth: baby_health
                        };
                        $.ajax({
                            type: "POST",
                            url: "{{route('checkmertanitysave')}}",
                            processData: false,
                            ...
                            success: function (response) {
                                if(response==200){
                                    clearAll();
                                    window.location.replace("{{route('checkmertanitypatient')}}");
                                }
                            },
                            error: function(response){
                                bootbox.alert({
