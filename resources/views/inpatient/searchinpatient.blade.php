...
function validateNum(appNum){
    ...
    $.ajax({
    type: "POST",
    url: "{{route('validateAppNumInpatient')}}",
    processData: false,
    contentType: false,
    cache: false,
    data:data,
    ...
    success: function (appointment) {
        if(appointment.exist){
            ...
            $("#appt_num").text(appointment.appId);
            $("#appt_num_1").val(appointment.appId);
            $("#appNum").focus();
        }else{
            $("#details").fadeOut();
...
<div class="row">
    ...
    <div class="col-md-10">
        <div class="box box-success mt-5">
            ...
            <div class="box-body">
                <form class="pl-5 pr-5 pb-5" method="post" action="{{route('checkInPatient')}}">
                    @csrf
                    ...
                    <input disabled id="btn_submit" type="submit" class="btn btn-success btn-lg mt-3 text-center"
